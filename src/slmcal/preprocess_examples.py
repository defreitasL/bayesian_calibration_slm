...
from .preprocess import PreprocessResultMoose2D, preprocess_legacy_moose2d, preprocess_legacy_millerDean, preprocess_legacy_no_index
from .features import add_ewma_forcings, add_movavg_forcings
...
def build_datasets_MoE(
        ...
        use_ewma: bool = False, 
        ewma_keys: tuple[str, ...] | None = None, 
        ewma_taus: tuple[float, ...] | None = None,
        use_movavg: bool = False,
        movavg_keys: tuple[str, ...] | None = None,
        movavg_windows: tuple[float, ...] | None = None,
        add_bmu_info: bool = False,
        ) -> tuple[TimeSeriesDataset, TimeSeriesDataset, TimeSeriesDataset]:
    # IMPORTANT:
    # Build the derived forcings on the *full available forcing record* first,
    # then compute EWMAs, and only afterwards crop to the requested analysis
    # window. This avoids the artificial spin-up at `start_date` that appears
    # when slow-varying covariates are computed on an already-truncated series.

    prep = preprocess_legacy_no_index(
        ds,
        end_date=end_date,
        params=md_params,
        cop_vars=True,
        add_bmu_info=add_bmu_info,
    )

    obs_mask = np.asarray(prep.obs_in_time_mask, dtype=bool)
    obs_time_hist = np.asarray(prep.time_obs)[obs_mask]
    obs_hist = np.asarray(prep.Obs, dtype=float)[obs_mask]
    idx_obs_hist = np.asarray(prep.idx_time_obs, dtype=int)[obs_mask]

    ds_hist = TimeSeriesDataset(
        time=prep.time,
        forcings={
            "E": prep.E,
            "P": prep.P,
        ...
        obs_time=obs_time_hist,
        obs=obs_hist,
        y0=(float(obs_hist[0]) if obs_hist.size > 0 else None),
        idx_obs=idx_obs_hist,
        dt=prep.dt,
        hberm=np.float64(prep.hberm),
    )

    if use_ewma:
        add_ewma_forcings(ds_hist, keys=ewma_keys, taus_days=ewma_taus, in_place=True)

    if use_movavg:
        add_movavg_forcings(ds_hist, keys=movavg_keys, windows_days=movavg_windows, in_place=True)

    time_hist = np.asarray(ds_hist.time).astype("datetime64[ns]")
    start64 = np.datetime64(np.datetime64(start_date, "ns"))
    split64 = np.datetime64(np.datetime64(split_date, "ns"))

    start_idx = int(np.searchsorted(time_hist, start64, side="left"))
    split_idx = int(np.searchsorted(time_hist, split64, side="left"))

    ds_full = ds_hist.subset(start_idx, time_hist.size)
    ds_cal = ds_hist.subset(start_idx, split_idx)

    return ds_cal, ds_full, ds_hist