...
# NOTE:
#   pySPADS originally relies on PyEMD (pure-python CEEMDAN) which can be slow.
#   This repo optionally bundles a compiled libeemd + pyeemd backend. We keep the
#   public API unchanged and auto-select the fastest available backend.
...
def decompose(
    ...
    # Backend selection:
    #   - default: "auto" -> use pyeemd if importable, otherwise PyEMD
    #   - override with env var PYSPADS_CEEMDAN_BACKEND in {"auto","pyeemd","pyemd"}
    backend = os.environ.get("PYSPADS_CEEMDAN_BACKEND", "auto").strip().lower()

    # Try pyeemd (compiled libeemd) first when allowed.
    if backend in {"auto", "pyeemd", "libeemd"}:
        try:
            try:
                import pyeemd  # type: ignore
            except Exception:
                # Monorepo convenience: if running from a repo checkout where
                # <repo>/pyeemd exists next to <repo>/src/, try to import it.
                import sys

                here = os.path.dirname(os.path.realpath(__file__))
                monorepo_pyeemd = os.path.realpath(os.path.join(here, "..", "..", "..", "pyeemd"))
                if os.path.isdir(monorepo_pyeemd) and monorepo_pyeemd not in sys.path:
                    sys.path.insert(0, monorepo_pyeemd)
                import pyeemd  # type: ignore

            # pyeemd.ceemdan signature differs from PyEMD CEEMDAN.
            # Map:
            #   num_trials -> ensemble_size
            ...
            with warnings.catch_warnings():
                ...
                imfs = pyeemd.ceemdan(
                    x,
                    ensemble_size=int(num_trials),
                    noise_strength=float(noise),
                    num_imfs=None,
                    rng_seed=0,
                )

            # pyeemd returns shape (n_imfs, n) -> match PyEMD output handling
            return pd.DataFrame(np.asarray(imfs, dtype=float).T, index=data.index)
        except Exception:
            if backend in {"pyeemd", "libeemd"}:
                raise

    # Fallback to PyEMD (original behaviour)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=DeprecationWarning)
        ...
        ceemd = CEEMDAN(
            trials=num_trials,
            epsilon=noise,
            processes=processes if parallel else None,
            parallel=parallel,
            parallel_kind=parallel_kind,
        )
        t0 = data.index[0]
        T = (data.index - t0) / np.timedelta64(1, "D")
        imfs = ceemd.ceemdan(data.to_numpy(), np.asarray(T, dtype=float), progress=progress)
